...
vote_governance_proposal() {
  ...
  if echo "$RESPONSE" | grep -q '"result":true'; then
    ...
    print_info "失败：admin RPC 未启用"
    print_info "请执行 选项 4 重新部署（已自动启用 admin RPC）"
  else
    print_info "投票失败：$RESPONSE"
  fi

  # 持久化环境变量
  if [ -f "$AZTEC_DIR/.env" ] && ! grep -q "GOVERNANCE_PROPOSER_PAYLOAD_ADDRESS" "$AZTEC_DIR/.env"; then
    echo "GOVERNANCE_PROPOSER_PAYLOAD_ADDRESS=\"$GOVERNANCE_PAYLOAD\"" >> "$AZTEC_DIR/.env"
    print_info "已写入环境变量（重启后生效）"
  fi
...
install_and_start_node() {
  ...
  validate_url Cons "$CONS_RPC" "CL RPC"
